...
VALUES
  ...
  -- QUESTION 01
Select count(*) as total_subscribed_people from subscriptions;
...
group by subscriptions.start_date;

SELECT subscriptions.start_date, COUNT(*) AS num_trials_started
FROM plans
JOIN subscriptions ON plans.plan_id = subscriptions.plan_id
...
With churn_per As (select COUNT(*) AS churn_count from subscriptions
join  plans ON subscriptions.plan_id = plans.plan_id
where plan_name = "churn"),
total_subs AS (
...
SELECT round (((churn_count * 100) / total_count),0) AS churn_percentage
