...
FROM subscriptions: Specifies the table from which we are retrieving data, in this case, the “subscriptions” table.
...
SELECT customer_id: Selects the customer_id column.
JOIN subscriptions AS s ON s.plan_id = p.plan_id: Joins the subscriptions table with the plans table based on the plan_id column.
WHERE p.plan_name IN (‘trial’, ‘churn’): Filters rows where the plan_name is either ‘trial’ or ‘churn’.
GROUP BY customer_id: Groups the data by customer_id.
HAVING COUNT(DISTINCT s.plan_id) = 2: Ensures that each customer has exactly two distinct plans.
...
WITH count_of_customers AS (
    ...
    FROM (
        ...
        WHERE p.plan_name IN ('trial', 'churn')
        GROUP BY customer_id
        HAVING COUNT(DISTINCT s.plan_id) = 2
           AND MIN(CASE WHEN p.plan_name = 'trial' THEN s.start_date END) < MIN(CASE WHEN p.plan_name = 'churn' THEN s.start_date END)
...
total_customers.total_count: Retrieves the total count of distinct customer_id values from the subscriptions table.
ROUND((COUNT(DISTINCT customer_id) * 100.0) / total_customers.total_count, 1): Calculates the percentage of customers for each plan, rounding the result to one decimal place.
FROM subscriptions s: Specifies the subscriptions table and assigns it an alias “s”.
...
ROUND((COUNT(customer_id)/(SELECT COUNT(DISTINCT customer_id) FROM LatestData))*100,1) as percent_of_clients
FROM LatestData
...
FROM subscriptions: Specifies the table from which we are retrieving data, in this case, the “subscriptions” table.
JOIN plans ON subscriptions.plan_id = plans.plan_id: Joins the “subscriptions” table with the “plans” table based on the common column “plan_id”. This allows us to access plan-related information for each subscription.
WHERE plans.plan_name = ‘pro annual’: Filters the data to include only subscriptions with the plan name ‘pro annual’.
AND subscriptions.start_date >= ‘2020–01–01’: Further filters the data to include only subscriptions that started on or after January 1, 2020.
...
SELECT 
    ROUND (AVG(DATEDIFF(annual_plan_date, join_date)), 0) AS average_days_to_upgrade
FROM 
...
JOIN plans AS p1 ON s1.plan_id = p1.plan_id: Joins the plans table with the ‘s1’ alias based on the plan_id to identify the ‘pro monthly’ plan.
JOIN plans AS p2 ON s2.plan_id = p2.plan_id: Joins the plans table with the ‘s2’ alias based on the plan_id to identify the ‘basic monthly’ plan.
...
JOIN plans AS p2 ON s2.plan_id = p2.plan_id
